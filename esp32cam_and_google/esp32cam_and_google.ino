...
int FLASH = 4; // Pin por el que vamos a controlar el encedido y apagado del led
...
void setup()
{
  ...
  pinMode(FLASH, OUTPUT);   // declaramos el pin como salida ( OUTPUT )
}

boolean enviar = true;

void loop() {
  digitalWrite (FLASH,HIGH);  // Encendemos el led
  saveCapturedImage();
  enviar = false;
  digitalWrite (FLASH,LOW);  // Apagamos el led
  delay(60000);
}
